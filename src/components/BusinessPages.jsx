...
const pricing = [{ name: "Monthly", price: "$49", label: "Flexible foundation", features: ["Monthly strategy review", "One priority channel", "Progress dashboard"] }, { name: "3 Months", price: "$129", label: "Build momentum", features: ["Quarterly growth plan", "Two priority channels", "Monthly performance review"] }, { name: "6 Months", price: "$239", label: "Recommended growth", features: ["Integrated marketing strategy", "Four priority channels", "Biweekly optimization"] }, { name: "12 Months", price: "$399", label: "Long-term partner", features: ["Full-funnel strategy", "Ongoing channel support", "Quarterly planning sessions"] }];
export function Pricing() { return <PageFrame eyebrow="Pricing" title="Choose the support that matches your ambition." intro="Every plan starts with the same clarity. Choose how much momentum you want to build together."><div className="pricing-grid">{pricing.map((plan) => <article className={`pricing-card ${plan.name === "6 Months" ? "featured" : ""}`} key={plan.name}>{plan.name === "6 Months" && <span className="best-badge">Most popular</span>}<span className="plan-label">{plan.label}</span><h2>{plan.name}</h2><strong>{plan.price}<small>/mo</small></strong><ul>{plan.features.map((feature) => <li key={feature}><Check size={14} />{feature}</li>)}</ul><Link className={plan.name === "6 Months" ? "primary-action" : "outline-button"} to="/contact">Get Started <ArrowRight size={15} /></Link></article>)}</div><p className="pricing-note"><ShieldCheck size={15} /> No fake payment processing. Your assessment helps us recommend the right fit before any conversation.</p></PageFrame>; }
